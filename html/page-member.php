...
$pdo = db_init();
try {
$stmt = $pdo->prepare("SELECT COUNT(*) AS hits FROM tbmkg_bbs WHERE class = (?)");
...
$stmt = $pdo->prepare("SELECT name, message, login_id, class_number, DATE_FORMAT(created,' %Y.%m.%d %a %H:%i:%S ') AS created FROM tbmkg_bbs WHERE class = (?) ORDER BY created DESC LIMIT {$offset},10");
$stmt->execute(array($class));
 ...
 exit;
  ...
  if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($isValidated == TRUE) {
      ...
      $stmt = $pdo->prepare("INSERT INTO tbmkg_bbs (name, message, class, login_id, class_number,created) VALUES (?, ?, ?, ?, ?, NOW())");
    ...
    <main>
        ...
        <section id="s-nav" class="section">
            <p><img src="<?php echo get_template_directory_uri(); ?>/images/point.gif" alt="">
                <?php echo h($class); ?>組　<?php echo h($name); ?>さんのページ 【ID:<?php echo h($login_id); ?>】 <a href="logout"><small>ログアウト</small></a></p>
        </section>
...
<a href="?prm=<?php echo $i; ?>"><?php echo $i; ?></a>
|
<?php endfor; ?>
</p>
...
<?php if($j < 3): ?>
<?php  $news_date[$j] = $post["created"]; ?>
<?php  $news[$j] = $post["message"]; ?>
<?php endif; ?>
<?php $j += 1;  ?>
  ...
  <?php endif; ?>
                ...
                <div class="pc-mode w30">
                  ...
                  <div class="member-text__right">
                    <h3>担任からのお知らせ</h3><br>
                    <ul>
                    <li><small><?php echo h($news_date[0]); ?></small><br>
                    <?php echo h($news[0]); ?></li>
                    <li><small><?php echo h($news_date[1]); ?></small><br>
                    <?php echo h($news[1]); ?></li>
                    <li><small><?php echo h($news_date[2]); ?></small><br>
                    <?php echo h($news[2]); ?></li>
                </div>
